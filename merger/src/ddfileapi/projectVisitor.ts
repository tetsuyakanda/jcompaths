...
import { createDiffFile, ProjectDiffFile } from './fileMethod';
...
export class ProjectVisitor {
  constructor(
    ...
    public m1: MethodListJsonData,
    public m2: MethodListJsonData
  ) {}
  ...
  private compareFiles(fs1: ProjectItemFileModel[], fs2: ProjectItemFileModel[], path: string[]) {
    ...
    return both.map(([f1, f2]) => createDiffFile(f1, f2, this.v1, this.v2, this.m1, this.m2, path));
  }
}
